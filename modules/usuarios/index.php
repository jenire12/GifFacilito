  ...
  <body>
    ...
    <div id="wrapper">
      ...
      <div id="page-wrapper">      	
          <div class="col-lg-12" >
          	<div class="row">
          		<h1>Usuarios <small>Lista de usuarios activos</small></h1>
	          	<ol class="breadcrumb">
	              <li><a href="../../"><i class="fa fa-globe"></i> Inicio</a></li>
	              <li class="active"><i class="fa fa-wrench"></i> Configuración</li>
	              <li class="active"><i class="fa fa-table"></i> Usuarios</li>
	            </ol>	
          	</div>   
            <div class="row">
            	<div class="col-lg-2 col-lg-offset-10">
	            	<a class="btn btn-primary pull-right" href="./usuarios_agregar.php" data-toggle="modal" data-target="#ModelUsuarios"> <span class="glyphicon glyphicon-plus-sign"></span> Agregar usuario </a>
	            </div>	
            </div>
            <br />
            <div class="row">    	 
		    ...
		    <div class="row">
		    	<div class="table-responsive">	
	              <table id="example" class="table table-hover table-striped table-condensed tablesorter" cellspacing="0" width="100%">
	                <thead>
	                  <tr align="center">
	                    <th>Trabajador <i class="fa fa-sort"></i></th>
	                  ...
	                  <?php
	                  		$sql = "
	                  				SELECT 
										t1.login,
										CASE t1.nivel
										    WHEN 1::numeric THEN 'ADMINISTRADOR'::text
										    WHEN 2::numeric THEN 'CONSULTOR 1'::text
										    WHEN 3::numeric THEN 'CONSULTOR 2'::text
										    ELSE 'INACCESIBLE'::text
										END AS nivel,
										t2.nombres||' '||t2.apellidos AS datos_trabajador,
										t3.descripcion								
									FROM
										tsca_usuarios t1
										LEFT JOIN tsca_mttrabajador t2 ON t1.login = t2.id_ficha
										LEFT JOIN tsca_departamentos t3 ON t2.departamento = t3.cod_departamento
									ORDER BY nivel
	                  				";
							$result = ejecutarPDO("cnx_siceac", $sql); 
							foreach ($result as $fila) { ?>
								<tr>									
									<td title="<?php echo $fila['descripcion']; ?>"><?php echo $fila['datos_trabajador']; ?></td>
									<td><?php echo $fila['login']; ?></td>									
									<td><?php echo $fila['nivel']; ?></td>
									<td>																				
										<a class="btn btn-success" href="./usuarios_modificar.php?login=<?php echo $fila['login']; ?>" data-toggle="modal" data-target="#ModelUsuarios"><span class="glyphicon glyphicon-edit"> Editar</span> </a>
										<a class="btn btn-danger" href="./usuarios_eliminar.php?login=<?php echo $fila['login']; ?>"> Eliminar</span> </a>
									</td>
								</tr>
							<?php } ?>
	                </tbody>
          ...
          <!-- Modal -->
			<div class="modal fade" id="ModelUsuarios" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
